...
function draw() {
  ...
  text("headspace", width * 0.4, height / 1.82);
  ...
  if (overlayVisible) {
    ...
    let targetSize = map(speed, 0, 40, 350, 650, true); // speed 0 --> 350px, speed 50+ --> 600px

    // smooth transition: increasing by 10% each frame
    circleSize = lerp(circleSize, targetSize, 0.1);

    overlay.clear();
    overlay.background(255, 192, 203);
    overlay.erase();
    overlay.ellipse(mouseX, mouseY, circleSize);
    overlay.noErase();
